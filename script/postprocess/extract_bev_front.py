...
from find_bag_by_tag import get_all_bags
...
def get_images_from_clips(bag_path):
    ...
    for clip in clips:
        for item in vision_folder_list:
            image_folder_path = os.path.join(bag_path, "clips", clip)
...
def frame_extraction(source_bag_dir, target_dir):
    ...
    for item in vision_folder_list:
        ...
        for image in selected_images[item]:
            ...
            index = index + 1
            target_folder = os.path.join(target_dir, os.path.basename(source_bag_dir) + '_' + clip_index)
            if not os.path.exists(target_folder):
                os.makedirs(target_folder)
...
if __name__ == "__main__":
    target_root = "/media/geely/BCCJ5t/FS-PLD/S08"
    if not os.path.exists(target_root):
        os.makedirs(target_root)

    bag_root = "/home/geely/nas/PLD-S6C/J6M-S08/"
    total_bag_dirs = get_all_bags(bag_root)
