...
def load_boxes(json_path):
    ...
    for box in box_json_data:
        ...
        # # if ("v0" not in obj_attr):
        # #     continue
        # # if obj_attr.split("__")[0] != "v0":
        # #     continue
        # obj_type_s = obj_type.split('__')
        # class_name = None
        # if obj_type_s[0] in CLASSES:
        #     class_name = CLASSES[obj_type_s[0]]
        # elif len(obj_type_s) > 1 and  obj_type_s[1] in CLASSES:
        #     class_name = CLASSES[obj_type_s[1]]
        # elif obj_type in CLASSES:
        #     class_name = CLASSES[obj_type]
        # else:
        #     print(obj_type, obj_type_s)
        #     continue
        #     # exit()
        ...
        #     continue
        # if x > 4 or x < -6 or y < 0:
        #     continue

        # if class_name in ["traffic_cone", "noparking_pillar", "noparking_board"]:
        #     z += 0.1 # 防止遮挡
        boxes.append([x,y,z,l,w,h,yaw])
        # labels.append(class_name)
        # box_vis_flags.append(obj_attr.split("__")[0])
    # if boxes == []:
...
# def point_project_distort(coords, image, M1, M2, D):
...
#     rvec = cv2.Rodrigues(M2[:3,:3])[0]
#     tvec = M2[:3,3]
#     points = cv2.fisheye.projectPoints(points.reshape(-1, 1, 3), rvec, tvec, M1[:3,:3], D)[0].reshape(-1, 2)

#     vaild = (points[:, 0] >=0) & (points[:, 0] < resolution[1])  & (points[:, 1] >=0) & (points[:, 1] < resolution[0]) & (zzz >0)  

#     return points[vaild],zzz[vaild]


def point_project_distort(coords, image, intr, extr, D=None):
...
def visualize_box(frame_path, save_path, params, boxes, box_oclu_infos=None,  undistort_flag=True):
    ...
    # cv2.imwrite(os.path.join(save_path,"vis_box", frame+"_4v.jpg"), img_save_4v)

    # cv2.imwrite(os.path.join(save_path,"vis_box", frame+"_far.jpg"), pics[10])
...
def get_color_from_intensity(point_intensity):
    ...
    return intensity_colors[int(point_intensity * len(intensity_colors) / 255)]

def visualize_pcd(frame_path, save_path, params, points, intensity,  undistort_flag=True):
    # print("********************"+os.path.basename(frame_path)+"********************")
    frame = os.path.basename(frame_path)
    pics = []
    ...
    for cam in cam_dict.keys():
        ...
        intrinsic = copy.deepcopy(params[cam_value]["intr"])
        # try:
        #     intrinsic = copy.deepcopy(params[cam_value]["origin_intr"])
        # except:
        #     intrinsic = copy.deepcopy(params[cam_value]["intr"])
        extrinsic = copy.deepcopy(params[cam_value]["extr"])
        D = params[cam_value]["D"]
        ...
        if (not undistort_flag) and "a_" in cam:
            balance = 0.5
            # new_intrinsic = cv2.fisheye.estimateNewCameraMatrixForUndistortRectify(intrinsic, D[:4], (img_height,img_width),None, None, balance)
            # map1, map2 = cv2.fisheye.initUndistortRectifyMap(intrinsic,D[:4],None,new_intrinsic,(img_height,img_width),cv2.CV_16SC2)
            # img = cv2.remap(img, map1, map2, interpolation=cv2.INTER_LINEAR)
            # coords, zzz = point_project_distort(copy.deepcopy(points), img, new_intrinsic, extrinsic)

            # img = cv2.fisheye.undistortImage(img, intrinsic, D[:4], None, intrinsic)
            # coords, zzz = point_project_distort(copy.deepcopy(points), img, new_intr, extrinsic, None)


            coords, zzz = point_project_distort(copy.deepcopy(points), img, new_intr, extrinsic, D[:4])

        else:
            coords, zzz = point_project(copy.deepcopy(points), img, new_intr, extrinsic)
        ...
        if coords is not None:
            ...
            for index in range(coords.shape[0]):
                ...
                cv2.drawMarker(img,position=p,color=get_color(zzz[index]),markerSize = circle_thickness*2, markerType=cv2.MARKER_CROSS, thickness=circle_thickness)
                # cv2.drawMarker(img,position=p,color=get_color_from_intensity(intensity[index]),markerSize = circle_thickness*2, markerType=cv2.MARKER_CROSS, thickness=circle_thickness)
...
def process_by_frame(frame, bag_dir_path, save_path, params, vis_box=False, vis_pcd=False, vis_box_in_pcd=False):
    ...
    if vis_pcd:
        visualize_pcd(frame_path, save_path, params, points, intensity, False)
    if vis_box_in_pcd:
