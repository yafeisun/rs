...
for frame, middle_ts, camera_ts_list in sync_data:
    # 检查 middle_ts 是否在 lidar_undist 中存在
...
转换完成后应验证:
1. `lidar_undist/*.pcd` 文件数 = `egopose_optpose/*.json` 文件数 = `result.json` 条目数
2. 所有 `egopose_optpose/*.json` 的时间戳都能在 `lidar_undist` 中找到对应 PCD
...
A: 检查以下对应关系：
1. `result.json` 中的 PCD 时间戳与 `lidar_undist` 文件一致
2. `egopose_optpose` JSON 文件名与 `lidar_undist` PCD 文件名一致
