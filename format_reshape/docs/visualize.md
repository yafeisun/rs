...
| 数据 | 坐标系 |
|------|--------|
| `result/test_calibration/middle/*.pcd` | **Body FLU**（X前，Y左，Z上） |
| `sensor_data/lidar/lidar_undist/*.pcd` | **Body FLU**（X前，Y左，Z上） |
| `calib_anno/*.json` 中的 `extrinsic` | **camera → Body FLU** |
| `calib_anno_vc/*.json` 中的 `extrinsic` | **camera → Body FLU** |

> **说明**：速腾源数据（PCD和相机的CameraExt）已经是 Body FLU 坐标系，
> 转换后直接拷贝使用，无需任何坐标变换。

**投影链**：
```
PCD (Body FLU) → [inv(extrinsic_cam_to_FLU)] → 相机坐标系 → 像素
```
...
- **基准路径**: 转换输出目录，例如 `/path/to/0203select_output/605/2025-11-21-16-15-45/`
- **点云文件 (PCD)**: `sensor_data/lidar/lidar_undist/*.pcd`（Body FLU 坐标系）
...
- **映射关系**: `node_output/peral-dataproc/result.json`
...
- 点云：`result/test_calibration/middle/*.pcd`（Body FLU 坐标系）

速腾 `calib.json` 生成方式：
- 来源：`car_*.yaml` 中各相机的 `CameraExt`（roll/pitch/yaw，FLU body 系）
- 工具：`pipeline/sensor_sync_data/convert_yaml_2_json.py`
- extr 含义：**body_FLU → camera**

---

## 注意事项

1. **点云坐标系**：`lidar_undist` PCD 已经是 Body FLU 坐标系，无需任何坐标变换。

2. **外参方向**：`calib_anno` 存的是 camera → Body FLU（正向），投影时需取逆得到 Body FLU → camera。
